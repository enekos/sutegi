...
- **Adapters** — all the framework/IO code:
  - *inbound* drive the app: `inbound_http.rs` (routes → use cases),
    `inbound_ai.rs` (an AI `Tool` → the **same** use case).
  - *outbound* are driven by it: `outbound_memory.rs`, `outbound_sqlite.rs` —
    interchangeable because both implement `TodoRepository`.
...
- **`respond` / `respond_created`** — map an `AppResult<T>` to a `Response`
  ...
  ```rust
  .post("/todos", "Create", move |req, _p| respond_created(create.execute(title)))
  ```
...
- **Swap adapters freely.** `REPO=memory` for tests and local dev, `REPO=sqlite`
  in prod — the use cases don't change. (When the Postgres adapter lands, it's a
  third implementation of the same port.)
...
| adapter tests | SQL mapping, HTTP shape | SQLite / a live server |
...
src/
  ...
  adapters/
    ...
    inbound_ai.rs      # impl Tool over a use case
    outbound_memory.rs # impl TheRepository (tests/dev)
    outbound_sqlite.rs # impl TheRepository (prod)
